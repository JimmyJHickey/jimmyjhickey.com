...
We need to find the PDF of $Y$. We can do this through the CDF of $X$.
...
F_Y(y) & = P(Y \leq y) \\
	&= P( X^\theta \leq y) \\
	& = P( \theta \log(X) \leq \log(y)) \\
	& = P(\log(X) \leq \frac{ 1 }{ \theta } \log(y)) \\
	& = P(X \leq y^{1/\theta}) \\
	& = F_X(y^{1/\theta}) \\ \\
f(y) & = F_X(y^{1/\theta}) \frac{ 1 }{ \theta } y^{1/\theta - 1} \\
...
$$
k < \frac{ f(y|2) }{ f(y|1) } = \frac{ \frac{ 1 }{ 2 } y^{1/2 - 1} e^{y^{-1/2}} }{ \frac{ 1 }{ 1 } y^{1/1 - 1} e^{y^-1/1} } = \frac{ 1 }{ 2 } y^{-1/2} e^{y-y^{1/2}}.
$$
